 ...
 * @Last modified time: 2020-02-13T17:23:17+00:00
 */
...
export default class Review extends Component{
  ...
  onSubmit = e => {
    ...
    axios.put(process.env.REACT_APP_BACKEND + `/shows/${id}`, newReview)
   ...
   componentDidMount(){
     const { id } = this.props.match.params;
     axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtToken');
     axios.get(process.env.REACT_APP_BACKEND + `/shows/${id}`).then((result) => {
       console.log(result);
       this.setState({
         show: result.data
