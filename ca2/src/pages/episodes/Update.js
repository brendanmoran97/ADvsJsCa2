 ...
 * @Last modified time: 2020-02-13T17:23:19+00:00
 */
...
export default class EpisodeUpdate extends Component{
  ...
  onSubmit = e => {
    ...
    axios.put(process.env.REACT_APP_BACKEND + `/episodes/${id}`, episode)
   ...
   componentDidMount(){
     const { id } = this.props.match.params;
           ...
           fetch(process.env.REACT_APP_BACKEND + '/shows')
             .then(res => res.json())
             .then((data) => {
               this.setState({
                 shows: data,
     ...
     axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtToken');
     axios.get(process.env.REACT_APP_BACKEND + `/episodes/${id}`).then((result) => {
       console.log(result);
       this.setState({
